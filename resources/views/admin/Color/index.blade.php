...
@section('content')
		...
		<div class="page-wrapper">
			<div class="page-content">
				...
				<!--end breadcrumb-->
				<h6 class="mb-0 text-uppercase">Colors</h6>
				<!-- Button trigger modal -->
				<button type="button" class="btn btn-primary" id="createColorBtn" data-bs-toggle="modal" data-bs-target="#colorModal">Add Color</button>
				<!-- Color Modal -->
				<div class="modal fade" id="colorModal" tabindex="-1" aria-labelledby="colorModalLabel" aria-hidden="true">
					<div class="modal-dialog">
						<div class="modal-content">
							<form id="colorForm" action="{{route('admin.color.store')}}" enctype="multipart/form-data">
								@csrf
								...
								<div class="modal-body">
									...
									<div class="mb-3">
										...
										<textarea class="form-control" id="color_name" name="name" rows="3" required></textarea>
									</div>

									<div class="mb-3">
										<label for="color_code" class="form-label">Color Code</label>
										<input type="text" class="form-control" id="color_code" name="code" required>
									</div>
								</div>
							...
							<table id="colorTable" class="table table-striped table-bordered">
								...
								<tbody>
									...
									<tr>
										<td>{{ $index+1 }}</td>
										<td>{{ $color->name }}</td>
										<td><a href="{{ $color->code }}" target="_blank">{{ $color->code }}</a></td>
										<td>{{ $color->created_at->format('M d, Y') }}</td>
										<td>
											<button class="btn btn-sm btn-primary editColorBtn" 
													data-color='@json($color)'>
												Edit
											</button>
											<button class="btn btn-sm btn-danger deleteColorBtn" 
													data-id="{{ $color->id }}">
												Delete
											</button>
										</td>
...
@section('page-script')
	...
	<script>
		...
		$(document).ready(function() {
			...
			$("#createColorBtn").on("click", function () {
